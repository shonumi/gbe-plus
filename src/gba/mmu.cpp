...
bool AGB_MMU::load_backup(std::string filename)
{
	...
	else if(current_save_type == JUKEBOX_CONFIG)
	{
		...
		jukebox.io_regs[0x008C] = (save_data[15] << 8) | save_data[14];
		jukebox.io_regs[0x009B] = (save_data[17] << 8) | save_data[16];
...
bool AGB_MMU::save_backup(std::string filename)
{
	...
	else if(current_save_type == JUKEBOX_CONFIG)
	{
		...
		cfg_data[12] = jukebox.io_regs[0x8A] & 0xFF;
		cfg_data[13] = (jukebox.io_regs[0x8A] >> 8) & 0xFF;

		cfg_data[14] = jukebox.io_regs[0x8C] & 0xFF;
		cfg_data[15] = (jukebox.io_regs[0x8C] >> 8) & 0xFF;

		cfg_data[16] = jukebox.io_regs[0x9B] & 0xFF;
		cfg_data[17] = (jukebox.io_regs[0x9B] >> 8) & 0xFF;

		file.write(reinterpret_cast<char*> (&cfg_data[0]), 0x12);
		file.close();
