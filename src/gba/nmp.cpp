...
void AGB_MMU::process_nmp_cmd()
{
	...
	switch(play_yan.cmd)
	{
		...
		case 0x8001:
			...
			//Manually trigger next IRQ (16-byte status read)
			if(play_yan.irq_count == 0)
			{
			...
			//Automatically and immediately trigger Headphone Status command
			else
			{
				play_yan.cmd = 0x8600;
				play_yan.irq_count = 0;
...
void AGB_MMU::access_nmp_io()
{
	...
	if((play_yan.access_param) && (play_yan.access_param != 0x101))
	{
		...
		{
			//Process manually triggered Game Pak IRQs (generated by the NMP itself)
			if(play_yan.nmp_manual_irq)
			{
				play_yan.nmp_manual_irq = false;
				play_yan.op_state = PLAY_YAN_STATUS;
				stat_data = play_yan.cmd;
				process_nmp_cmd();
			}
