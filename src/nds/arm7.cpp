		...
		if(check_condition(instruction_pipeline[pipeline_id]))
		{
			switch(instruction_operation[pipeline_id])
			{
				...
				default:
					...
					std::cout<<"CPU::ARM7::Error - Unknown ARM instruction -> 0x" << std::hex << debug_code << "\n";
					if(!config::ignore_illegal_opcodes) { running = false; }
					break;
			}
		}
...
void NTR_ARM7::update_condition_arithmetic(u32 input, u32 operand, u32 result, bool addition, s64 carry_op)
{
