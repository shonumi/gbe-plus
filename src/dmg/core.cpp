...
bool DMG_core::get_save_state_info(u32 offset, std::string filename)
{
	u32 version = 0;
	u8 system_type = 0;
	u8 state_date[32];
...
bool DMG_core::set_save_state_info(std::string filename)
{
...
void DMG_core::handle_hotkey(SDL_Event& event)
{
	...
	else if((event.type == SDL_KEYDOWN) && (event.key.keysym.sym == SDLK_F4))
	{
		...
		{
			case GB_KISS_LINK:
				core_mmu.kiss_link.mode++;
				core_mmu.kiss_link.mode &= 0x3;

				switch(core_mmu.kiss_link.mode)
				{
